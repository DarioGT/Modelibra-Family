...
public class RemoveCommand extends Command {
	...
	public RemoveCommand(Transaction aTransaction, EntityModel parent,
	...
	private void basicDo() {
		try {
			...
			System.out.println("basicDo: " + parent.getName() + ", remove "
	...
	private void basicUndo() {
		try {
			...
			System.out.println("basicUndo: " + parent.getName() + ", add "
					+ child.getName() + ", " + e.getMessage());
		}
	}

	/*
	 * private void executeRemoveCommand() { log(); basicDo(); if (removed)
	 * parent.notifyObservers(new ModelEvent("remove", parentChildrenName,
	 * child)); if (detached) child.notifyObservers(new ModelEvent("detach",
	 * childParentName, parent)); }
	 */

	private void undoRemoveCommand() {
		basicUndo();
		if (added)
	...
	private void redoRemoveCommand() {
		basicDo();
		if (removed)
			parent.notifyObservers(new ModelEvent("remove", parentChildrenName,
					child));
