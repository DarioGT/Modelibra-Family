...
public class Member extends GenMember {
	...
	/**
	 * Checks if a member has a valid role (regular or admin).
	 * 
	 * @return <code>true</code> if the member has a valid role
	 */
	protected boolean hasValidRole() {
		boolean valid = false;
		if ((getRole().equals("regular")) || (getRole().equals("admin"))) {
			valid = true;
		}
		return valid;
	}
