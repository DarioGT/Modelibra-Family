...
public class AddAction<T extends IEntity<T>> extends EntitiesAction<T> {

	public AddAction(IEntities<T> entities, T entity) {
	...
	public boolean undo() {
		if (isExecuted()) {
			boolean undone = getEntities().remove(getEntity());
			if (undone) {
				setStatus("undone");
			} else {
				setStatus("error");
