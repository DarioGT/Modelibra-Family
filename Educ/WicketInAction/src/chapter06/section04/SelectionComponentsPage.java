	...
	public SelectionComponentsPage() {
		...
		class MilkTypesModel extends LoadableDetachableModel {
			...
			protected Object load() {
				return Arrays.asList("Bison", "Camel", "Cow", "Goat",
						"Reindeer", "Sheep", "Yak");
			}
		...
		add(new ListChoice("category1", new PropertyModel(cheese, "category"),
				categories));
		cheese.setCategory("Blue vein");

		add(new DropDownChoice("category2", new PropertyModel(cheese,
				"category"), categories));

		add(new RadioChoice("category3", new PropertyModel(cheese, "category"),
				categories).setSuffix(" "));

		/* Section 7.4.2 */
		List<String> choices = Arrays.asList("Camel", "Cow", "Goat",
				"Reindeer", "Sheep", "Yak");
		add(new ListMultipleChoice("milkTypes1", new PropertyModel(cheese,
				"milkTypes"), // #1
				choices)); // #2
		...
		add(new CheckBoxMultipleChoice("milkTypes2", new PropertyModel(cheese,
				"milkTypes"), choices).setSuffix(" "));

		/* Section 7.4.3 */

		List<MilkType> milktypes = Arrays.asList(new MilkType(1, "Camel"),
				new MilkType(2, "Cow"), new MilkType(3, "Goat"));
		add(new DropDownChoice("milkTypes3", new PropertyModel(cheese,
				"milkType"), milktypes, new ChoiceRenderer("name", "id")));
		...
		Form form = new Form("form744", new CompoundPropertyModel(searchPars)) {
			...
			protected void onSubmit() {
				// perform search for cheeses based on the search
				// criteria
			}
		};
		add(form);
		...
		form.add(wmc);
		form
				.add(new AjaxCheckBox("advanced", new PropertyModel(wmc,
						"visible")) {
					@Override
