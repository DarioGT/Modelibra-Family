...
public class Members extends GenMembers {
	...
	protected boolean preRemove(Member member) {
		boolean pre = true;
		try {
			if (super.preRemove(member)) {
				Questions memberQuestions = member.getQuestions();
				if (memberQuestions.size() > 0) {
					pre = false;
					getErrors()
							.add("Member.remove.validation",
									"This member cannot be removed since she has some questions.");
				}
			} else {
				pre = false;
			}
		} catch (Exception e) {
			log.error("Error in Members.preRemove: " + e.getMessage());
		}
		return pre;
	}
