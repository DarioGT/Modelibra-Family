...
public abstract class GenUrls extends Entities<Url> {
	...
	private static final long serialVersionUID = 1192739599326L;
	...
	/**
	 * Gets urls ordered by webLink.
	 * 
	 * @param ascending
	 *            <code>true</code> if ascending
	 * @return ordered urls
	 */
	public Urls getUrlsOrderedByWebLink(boolean ascending) {
		return getUrls("webLink", ascending);
	}
	 ...
	 * @return url
	 */
	public Url createUrl(Place placeParent, String name, String webLink) {
		Url url = new Url(getModel());
		url.setPlace(placeParent);
		url.setName(name);
		url.setWebLink(webLink);
		if (!add(url)) {
			url = null;
		}
		return url;
