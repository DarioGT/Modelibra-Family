...
public class HomePageMenuPanel extends DmPanel {
	...
	public HomePageMenuPanel(final String wicketId, DmPage dmPage) {
		...
		try {
			...
			// placesView.setContextPage(dmPage);
			placesView.setPage(dmPage);
			placesView.setUpdate(true);

			Link placeUpdateTableLink = EntityUpdateTablePage.link(
			...
			Travelers travelersOrderedByLastFirstName = travelers
					.getTravelersOrderedByLastFirstName(true);
			...
			// travelersView.setContextPage(dmPage);
			travelersView.setPage(dmPage);
			travelersView.setUpdate(true);

			Link travelerUpdateTableLink = EntityUpdateTablePage.link(
					travelersViewModel, travelersView);
			add(travelerUpdateTableLink);

			// Update Messages of the signed in traveler
			...
			if (traveler != null) {
				...
				// travelerMessagesView.setContextPage(dmPage);
				travelerMessagesView.setPage(dmPage);
				travelerMessagesView.setUpdate(true);
			}
			Link messageUpdateTableLink = EntityUpdateTablePage.link(
					travelerMessagesViewModel, travelerMessagesView);
			...
			defaultLanguage = (CountryLanguage) languages
			...
			languageViewModel.getUserProperties().addUserProperty(
			...
			Panel languageChoicePanel = new CountryLanguageChoicePanel(
					languageViewModel, countryLanguageView);
